...
class Psi
{
  public:
    ...
    Psi(const size_t nk_in, const size_t nbd_in, const size_t nbs_in, const int* ngk_in = nullptr, const bool k_first_in = true);
    // Constructor 4: copy a new Psi which have several k-points and several bands from inputted psi_in
    Psi(const Psi& psi_in, const size_t nk_in, size_t nband_in = 0);
    // Constructor 5: a wrapper of a data pointer, used for Operator::hPsi()
    // in this case, fix_k can not be used
    Psi(T* psi_pointer, const Psi& psi_in, const size_t nk_in, size_t nband_in = 0);
    ...
    Psi(T* psi_pointer, const size_t nk_in, const size_t nbd_in, const size_t nbs_in, const int* ngk_in = nullptr, const bool k_first_in = true);
    // Destructor for deleting the psi array manually
    ~Psi();

    // allocate psi for three dimensions
    void resize(const size_t nks_in, const size_t nbands_in, const size_t nbasis_in);
    ...
    void fix_k(const size_t ik) const;
    ...
    void fix_b(const size_t ib) const;
    ...
    const int get_ngk(const size_t ik_in) const;
    ...
    std::tuple<const T*, size_t> to_range(const Range& range) const;
    int npol = 1;
  ...
  private:
    ...
    // dimensions
    size_t nk = 1; // number of k points
    size_t nbands = 1; // number of bands
    size_t nbasis = 1; // number of basis

    mutable size_t current_k = 0; // current k point
    mutable size_t current_b = 0; // current band index
    mutable size_t current_nbasis = 1; // current number of basis of current_k

                           ...
                           const double& sparse_thr)
{
    ...
    sparse_format::cal_STN_R_for_T(ucell, pv, HS_Arrays, grid, sparse_thr, "T");

    return;
}

void sparse_format::cal_pR(const UnitCell& ucell,
                           const Parallel_Orbitals& pv,
                           LCAO_HS_Arrays& HS_Arrays,
                           Grid_Driver& grid,
                           const TwoCenterBundle& two_center_bundle,
                           const double& sparse_thr)
{
    ...
    assert(nnr >= 0);
    fsr_tmp.DSloc_Rx = new double[nnr];
    fsr_tmp.DSloc_Ry = new double[nnr];
    fsr_tmp.DSloc_Rz = new double[nnr];
    int beg = 0;
    int len = nnr;
    ModuleBase::GlobalFunc::ZEROS(fsr_tmp.DSloc_Rx + beg, len);
    ModuleBase::GlobalFunc::ZEROS(fsr_tmp.DSloc_Ry + beg, len);
    ModuleBase::GlobalFunc::ZEROS(fsr_tmp.DSloc_Rz + beg, len);
    ...
    sparse_format::cal_STN_R_for_T(ucell, pv, HS_Arrays, grid, sparse_thr, "p");

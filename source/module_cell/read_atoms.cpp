...
int UnitCell::read_atom_species(std::ifstream& ifa, std::ofstream& ofs_running)
{
    ...
    if (ModuleBase::GlobalFunc::SCAN_BEGIN(ifa, "ATOMIC_SPECIES"))
    {
        ...
        for (int i = 0; i < ntype; i++)
        {
            ...
            if (!PARAM.inp.use_paw)
            {
                ...
                {
                    if (one_string == "auto" || one_string == "upf" || one_string == "vwr" || one_string == "upf201"
                        || one_string == "blps")
                    {
                    ...
                    {
                        GlobalV::ofs_warning << "unrecongnized pseudopotential type: " << one_string
                                             << ", check your STRU file." << std::endl;
                ...
                {
                    ofs_running << "\n"
                                << std::setw(6) << atom_label[i] << std::setw(12) << atom_mass[i] << std::setw(18)
                                << pseudo_fn[i] << std::setw(18) << pseudo_type[i];
                }

                // Peize Lin test for bsse 2021.04.07
                const std::string bsse_label = "empty";
                this->atoms[i].flag_empty_element
                    = (search(atom_label[i].begin(), atom_label[i].end(), bsse_label.begin(), bsse_label.end())
                       != atom_label[i].end())
                          ? true
                          : false;
            }
    ...
    if (GlobalC::exx_info.info_global.cal_exx || PARAM.inp.rpa)
    {
        ...
        if (ModuleBase::GlobalFunc::SCAN_BEGIN(ifa, "Shrink_ABFS_ORBITAL"))
        {
            for (int i = 0; i < ntype; i++)
            {
                std::string ofile;
                ifa >> ofile;
                GlobalC::exx_info.info_ri.files_shrink_abfs.push_back(ofile);
            }
        }
    }
...
bool UnitCell::read_atom_positions(std::ifstream& ifpos, std::ofstream& ofs_running, std::ofstream& ofs_warning)
{
    ...
    if (ModuleBase::GlobalFunc::SCAN_BEGIN(ifpos, "ATOMIC_POSITIONS"))
    {
        ...
        if (Coordinate != "Cartesian" && Coordinate != "Direct" && Coordinate != "Cartesian_angstrom"
            && Coordinate != "Cartesian_au" && Coordinate != "Cartesian_angstrom_center_xy"
            && Coordinate != "Cartesian_angstrom_center_xz" && Coordinate != "Cartesian_angstrom_center_yz"
        ...
        for (int it = 0; it < ntype; it++)
        {
            ...
            {
                ofs_warning << " Label orders in ATOMIC_POSITIONS and ATOMIC_SPECIES sections do not match!"
                            << std::endl;
            ...
            if (na > 0)
            {
                ...
                for (int ia = 0; ia < na; ia++)
                {
                    ...
                    atoms[it].mag[ia]
                        = magnet.start_magnetization[it]; // if this line is used, default startmag_type would be 2
                    ...
                    if ((int)tmpid[0] < 0)
                    {
                        std::cout << "read_atom_positions, mismatch in atom number for atom type: " << atoms[it].label
                                  << std::endl;
                        exit(1);
                    }
                    ...
                    while ((tmpid != "\n") && (ifpos.good()) && (tmpid != "#"))
                    {
                        ...
                        else if (tmpid == "mag" || tmpid == "magmom")
                        {
                            ...
                            if ((tmp >= 48 && tmp <= 57) or tmp == '-')
                            {
                                ...
                                atoms[it].mag[ia] = sqrt(pow(atoms[it].m_loc_[ia].x, 2) + pow(atoms[it].m_loc_[ia].y, 2)
                    ...
                    if (PARAM.inp.nspin == 4)
                    {
                        if (PARAM.inp.noncolin)
                        {
                            ...
                            if (input_angle_mag)
                            {
                                atoms[it].m_loc_[ia].z = atoms[it].mag[ia] * cos(atoms[it].angle1[ia]);
                                if (sin(atoms[it].angle1[ia]) > 1e-10)
                                {
                                    atoms[it].m_loc_[ia].x
                                        = atoms[it].mag[ia] * sin(atoms[it].angle1[ia]) * cos(atoms[it].angle2[ia]);
                                    atoms[it].m_loc_[ia].y
                                        = atoms[it].mag[ia] * sin(atoms[it].angle1[ia]) * sin(atoms[it].angle2[ia]);
                                }
                            }
                    ...
                    if (Coordinate == "Cartesian" || Coordinate == "Cartesian_angstrom"
                        || Coordinate == "Cartesian_angstrom_center_xy" || Coordinate == "Cartesian_angstrom_center_xz"
                        || Coordinate == "Cartesian_angstrom_center_yz" || Coordinate == "Cartesian_angstrom_center_xyz"
                ...
                for (int I2 = 0; I2 < this->atoms[T2].na; I2++)
                {
                    ...
                    else
                    {
                        ...
                        if (norm < tolerence_bohr) // unit is Bohr
                        {
                            ...
                            GlobalV::ofs_warning << " type:" << this->atoms[T1].label << " atom " << I1 + 1
                                                 << std::endl;
                            GlobalV::ofs_warning << " type:" << this->atoms[T2].label << " atom " << I2 + 1
                                                 << std::endl;
                               ...
                               const int& iproc) const
{
    ...
    for (int it = 0; it < ntype; it++)
    {
        ...
        for (int ia = 0; ia < atoms[it].na; ia++)
        {
            ...
            {
                str += FmtCore::format(" v%20.10f%20.10f%20.10f",
                                       atoms[it].vel[ia].x,
                                       atoms[it].vel[ia].y,
                                       atoms[it].vel[ia].z);
            }
            ...
            else if (nspin == 4 && magmom) // output magnetic information
            {
                str += FmtCore::format(" mag%8.4f%8.4f%8.4f",
                                       atom_mulliken[nat_][1],
                                       atom_mulliken[nat_][2],
                                       atom_mulliken[nat_][3]);
            }
            str += "\n";
            nat_++;
        }
...
void UnitCell::print_tau() const
{
    ...
    // assert (direct || Coordinate == "Cartesian" || Coordinate == "Cartesian_angstrom"); // this line causes abort in
    // unittest ReadAtomPositionsCACXY. previously there are two if-statements, the first is `if(Coordinate ==
    // "Direct")` and the second is `if(Coordinate == "Cartesian" || Coordiante == "Cartesian_angstrom")` however the
    // Coordinate can also be value among Cartesian_angstrom_center_xy, Cartesian_angstrom_center_xz,
    // Cartesian_angstrom_center_yz and Cartesian_angstrom_center_xyz if Coordinate has value one of them, this
    // print_tau will not print anything.
    ...
    for (int it = 0; it < ntype; it++)
    {
        for (int ia = 0; ia < atoms[it].na; ia++)
        {
            ...
            const double& y = direct ? atoms[it].taud[ia].y : atoms[it].tau[ia].y;
            const double& z = direct ? atoms[it].taud[ia].z : atoms[it].tau[ia].z;
            table += FmtCore::format(
                "%5s%-s%-5d%20.10f%20.10f%20.10f%8.4f%20.10f%20.10f%20.10f\n", // I dont know why there must be a
                                                                               // redundant "tau[c|d]_" in the output.
                                                                               // So ugly, it should be removed!
                redundant_header,
                atoms[it].label,
                ia + 1,
                x,

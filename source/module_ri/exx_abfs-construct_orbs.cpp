    ...
    const double times_threshold)
{
    ...
    const std::vector<std::vector<std::vector<std::vector<double>>>> abfs_same_atom_orth_psi
        = orth(abfs_same_atom_psi, orbs, PARAM.inp.gs_orth_thr);
            ...
            for( int N1=0; N1!=orbs.Phi[T].getNchi(L1); ++N1 )
            {
                for( int L2=L1; L2<=orbs.Phi[T].getLmax(); ++L2 )
                {
                    for( int N2=((L2==L1)?N1:0); N2!=orbs.Phi[T].getNchi(L2); ++N2 )
                    {
                        ...
                        for( int ir=0; ir!=orbs.Phi[T].PhiLN(L1,N1).getNr(); ++ir)
                        {
                            mult_psir[ir] = orbs.Phi[T].PhiLN(L1,N1).getPsi_r(ir) *
orbs.Phi[T].PhiLN(L2,N2).getPsi_r(ir) ;
                        }
                        for( int L_new=std::abs(L1-L2); L_new<=L1+L2; ++L_new )
                        {
    ...
    const double times_threshold)
{
    ...
    const std::vector<std::vector<std::pair<std::vector<double>, RI::Tensor<double>>>> eig
        = ABFs_Construct::PCA::cal_PCA(orbs, abfs, kmesh_times_mot);
        ...
        else
        {
            ModuleBase::WARNING(ModuleBase::GlobalFunc::TO_STRING(__FILE__),
                                "Element " + ModuleBase::GlobalFunc::TO_STRING(T)
                                    + " , all training data (lcao[i]*lcao[j]) are all the same. So PCA randomly choose "

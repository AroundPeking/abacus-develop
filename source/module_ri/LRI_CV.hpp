                 ...
                 std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, 3>>>>
{
    ...
    std::map<TA, std::map<TAC, std::pair<RI::Tensor<Tdata>, std::array<RI::Tensor<Tdata>, 3>>>> Cs_dCs_tmp
        = this->cal_datas(ucell, list_A0, list_A1, flags, func_cal_Rcut, func_DPcal_C_dC);
    ...
    else
    {
        if ((ModuleBase::Vector3<double>(0, 0, 0) == R) && (it0 == it1))
        {
            ...
            if (GlobalC::exx_info.info_ri.exx_cs_inv_thr > 0)
                L = LRI_CV_Tools::cal_I(V, Inverse_Matrix<Tdata>::Method::syev, GlobalC::exx_info.info_ri.exx_cs_inv_thr);
            else

    ...
    const double kmesh_times)
{
    ...
    for (size_t T = 0; T != files_abfs.size(); ++T)
        abfs[T] = construct_abfs_T(files_abfs[T],
                                   T,
...
std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>> Exx_Abfs::IO::construct_abfs(
    ...
    const double kmesh_times)
{
    std::vector<std::vector<std::vector<Numerical_Orbital_Lm>>>&& abfs = construct_abfs(orbs, files_abfs, kmesh_times);
...
std::vector<std::vector<Numerical_Orbital_Lm>> Exx_Abfs::IO::construct_abfs_T(const std::string& file_name,
                                                                              const int& T,
                                                                              const int& nk,
                                                                              const double& dk,
                                                                              const double& dr_uniform)
{
    ...
    while (ifs.good())
    {
        ...
        else if ("Lmax" == word)
        {
            ...
            {
                std::stringstream ss;
                ss << "Lmax>=11 error in " << __FILE__ << " line " << __LINE__;
    ...
    for (size_t L = 0; L <= L_size; ++L)
    {
        for (size_t N = 0; N != N_size[L]; ++N)
        {
            ...
            double unit = 0.0;
            ModuleBase::Integral::Simpson_Integral(meshr,
                                                   ModuleBase::GlobalFunc::VECTOR_TO_PTR(inner),
    ...
    for (size_t L = 0; L <= L_size; ++L)
    {
        ...
        for (size_t N = 0; N != N_size[L]; ++N)
        {
            abfs_T[L][N].set_orbital_info(label,
                                          T,     // type

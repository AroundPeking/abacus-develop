...
void Exx_LRI<Tdata>::cal_exx_ions(const UnitCell& ucell, const bool write_cv)
{
    ...
    std::map<TA, std::map<TAC, RI::Tensor<Tdata>>> Vs
        = this->cv.cal_Vs(ucell, list_As_Vs.first, list_As_Vs.second[0], {{"writable_Vws", true}});
    ...
    if (this->info.use_ewald && this->info.ccp_type == Conv_Coulomb_Pot_K::Ccp_Type::Ccp)
    {
        ...
        std::map<TA, std::map<TAC, RI::Tensor<Tdata>>> Vs_full = this->evq.cal_Vs(ucell, chi, Vs);
    ...
    if (PARAM.inp.cal_force || PARAM.inp.cal_stress)
    {
        std::map<TA, std::map<TAC, std::array<RI::Tensor<Tdata>, Ndim>>> dVs
            = this->cv.cal_dVs(ucell, list_As_Vs.first, list_As_Vs.second[0], {{"writable_dVws", true}});
        ...
        if (this->info.use_ewald)
        {
            ...
            if (this->info.hybrid_beta)
            {
                dVs_sr = this->sr_cv.cal_dVs(ucell, list_As_Vs.first, list_As_Vs.second[0], {{"writable_dVws", true}});
                this->sr_cv.dVws = LRI_CV_Tools::get_dCVws(ucell, dVs_sr);
            }
            // const double chi = 1.0 / this->lambda;
            // dVs = this->evq.cal_dVs(chi, dVs);
            dVs = this->info.hybrid_beta ? LRI_CV_Tools::minus(dVs, dVs_sr) : dVs;
        }

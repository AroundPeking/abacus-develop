...
namespace RI_Util
{
inline std::array<int, 3> get_Born_vonKarmen_period(const K_Vectors& kv)
{
                                                       ...
                                                       const int nspin0)
{
    ...
    case 1: {
        double bvk_a1 = ucell.a1.norm() * p_kv->nmp[0];
        double bvk_a2 = ucell.a2.norm() * p_kv->nmp[1];
        double bvk_a3 = ucell.a3.norm() * p_kv->nmp[2];
    ...
    case Conv_Coulomb_Pot_K::Ccp_Type::Cam: { // Rcut_type = 1 is not supported (jiyy)
        return {{"hse_omega", info.hse_omega},
                ...
                {"hybrid_beta", info.hybrid_beta},
                {"Rcut_type", 0},
                {"hf_Rcut", hf_Rcut}};
    }
    case Conv_Coulomb_Pot_K::Ccp_Type::Ccp_Cam:
        return {{"hse_omega", info.hse_omega}, {"hybrid_alpha", info.hybrid_alpha}, {"hybrid_beta", info.hybrid_beta}};
    default:
        throw std::domain_error(std::string(__FILE__) + " line " + std::to_string(__LINE__));
        break;
    }

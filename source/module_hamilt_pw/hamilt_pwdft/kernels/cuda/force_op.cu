...
__global__ void cal_vkb1_nl(const int npwx,
                            const int vkb_nc,
                            const int nbasis,
                            const int ipol,
...
template <typename FPTYPE>
__global__ void cal_force_nl(const bool nondiagonal,
                             const int wg_nc,
                             const int ntype,
                             const int spin,
                             const int deeq_2,
            ...
            if (nondiagonal)
            {
                ...
                for (int ip2 = 0; ip2 < Nprojs; ip2++)
                {
                    if (ip != ip2)
                    {
                        ...
                        {
                            const FPTYPE dbb
                                = (conj(dbecp[ipol * nbands * nkb + ib * nkb + inkb]) * becp[ib * nkb + jnkb]).real();
                            atomicAdd(force + iat * forcenl_nc + ipol, -ps * fac * dbb);
                        }
                    }
                }
...
{
    cal_force_nl<FPTYPE>
        <<<nbands_occ * ntype, THREADS_PER_BLOCK>>>(nondiagonal,
                                                    wg_nc,
                                                    ntype,
                                                    spin,
                                                    deeq_2,
...
template <typename FPTYPE>
__global__ void cal_force_nl(const int wg_nc,
                             const int ntype,
                             const int deeq_2,
                             const int deeq_3,
                             const int deeq_4,
...
template <typename FPTYPE>
__global__ void saveVkbValues_(const int* gcar_zero_ptrs,
...
void saveVkbValues(const int* gcar_zero_ptrs,
                   ...
                   int npwx)
{
    saveVkbValues_<FPTYPE>
        <<<nkb * gcar_zero_count, THREADS_PER_BLOCK>>>(gcar_zero_ptrs,
...
template <typename FPTYPE>
__global__ void revertVkbValues_(const int* gcar_zero_ptrs,
...
// nspin==4 kernel for deltaSpin force
template <typename FPTYPE>
__global__ void cal_force_onsite(int wg_nc,
                                 int ntype,
                                 int forcenl_nc,
...
template void revertVkbValues<double>(const int* gcar_zero_ptrs,
                                      std::complex<double>* vkb_ptr,
                                      const std::complex<double>* vkb_save_ptr,
                                      int nkb,
                                      int gcar_zero_count,
                                      int npw,
                                      int ipol,
...
template void saveVkbValues<double>(const int* gcar_zero_ptrs,
                                    const std::complex<double>* vkb_ptr,
                                    std::complex<double>* vkb_save_ptr,
                                    int nkb,
                                    int gcar_zero_count,
                                    int npw,
                                    int ipol,

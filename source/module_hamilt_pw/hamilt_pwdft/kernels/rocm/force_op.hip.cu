...
{
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cal_vkb1_nl<FPTYPE>),
                       dim3(nkb),
...
void cal_force_nl_op<FPTYPE, base_device::DEVICE_GPU>::operator()(const base_device::DEVICE_GPU* ctx,
                                                                  ...
                                                                  FPTYPE* force)
{
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cal_force_nl<FPTYPE>),
...
void saveVkbValues(const int* gcar_zero_ptrs,
                   ...
                   int npwx)
{
    hipLaunchKernelGGL(HIP_KERNEL_NAME(saveVkbValues_<FPTYPE>),
                       dim3(nkb * gcar_zero_count),
                       dim3(THREADS_PER_BLOCK),
                       0,
                       0,
                       gcar_zero_ptrs,
...
{
    hipLaunchKernelGGL(HIP_KERNEL_NAME(revertVkbValues_<FPTYPE>),
                       dim3(nkb * gcar_zero_count),
                       dim3(THREADS_PER_BLOCK),
                       0,
                       0,

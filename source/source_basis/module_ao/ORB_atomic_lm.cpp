...
)
{
    ...
    case Psi_Type::Psik2:
        ...
        {
            throw std::domain_error("flag_sbpool false not finished in Numerical_Orbital_Lm::set_orbital_info_k. "
...
#pragma omp parallel for schedule(static)
    ...
    for (int ir = 0; ir < this->nr_uniform; ir++)
    {
        const double psi_uniform_tmp
            = ModuleBase::Mathzone_Add1::Uni_RadialF(ModuleBase::GlobalFunc::VECTOR_TO_PTR(this->psi),
                                                     this->nr,
                                                     this->rab[0],
                                                     ir * dr_uniform);
        this->psi_uniform[ir] = psi_uniform_tmp;
        //    	this->psi_uniform[ir] = ModuleBase::Mathzone::Polynomial_Interpolation(this->psi, this->nr,
        //    this->rab[0], ir * dr_uniform);
    }
    ...
    // std::cout<<"psi0="<<"  "<<this->psi[0]<<"  "<<"psi1="<<"  "<<this->psi[1]<<"  "<<"r1="<<"
    // "<<this->r_radial[1]<<std::endl; std::cout<<"yp1="<<"  "<<yp1<<std::endl; ModuleBase::Mathzone_Add1::SplineD2
    // (r_radial, psi, nr, yp1, 0.0, y2);
    ...
    {
    case 0:
        ModuleBase::Mathzone_Add1::SplineD2(ModuleBase::GlobalFunc::VECTOR_TO_PTR(r_radial),
    ...
    case 4:
        ...
        break;
    default:
        // GlobalV::ofs_warning << " The angular momentum larger than 4 (g orbitals) may be error about eggbox. " <<
        // std::endl; GlobalV::ofs_warning << " Check file " << __FILE__ << " line " << __LINE__ <<std::endl; std::cout
        // << " The angular momentum larger than 4 (g orbitals) may be error about eggbox. " << std::endl; std::cout <<
        // " Check file " << __FILE__ << " line " << __LINE__ <<std::endl;
        ModuleBase::Mathzone_Add1::SplineD2(ModuleBase::GlobalFunc::VECTOR_TO_PTR(r_radial),
                                            ModuleBase::GlobalFunc::VECTOR_TO_PTR(psi),
                                            nr,
                                            0.0,
    ...
    ModuleBase::Mathzone_Add1::Cubic_Spline_Interpolation(ModuleBase::GlobalFunc::VECTOR_TO_PTR(r_radial),
                                                          ModuleBase::GlobalFunc::VECTOR_TO_PTR(psi),
                                                          y2,
                                                          nr,
                                                          rad,
    ...
    {
        ModuleBase::Mathzone_Add1::Uni_Deriv_Phi(ModuleBase::GlobalFunc::VECTOR_TO_PTR(this->psi_uniform),
                                                 this->nr_uniform,
                                                 dr_uniform,
                                                 angular_momentum_l,
                                                 tmp);
...
void Numerical_Orbital_Lm::use_uniform(const double &dr_uniform_in)
{
    ...
    {
        this->psi_uniform[i] =
            ModuleBase::Mathzone_Add1::Uni_RadialF(ModuleBase::GlobalFunc::VECTOR_TO_PTR(psi), this->nr, this->rab[0],
i*dr_uniform);
    }
...
void Numerical_Orbital_Lm::cal_kradial(void)
{
    ...
    */
    ...
    for (int ik = 0; ik < nk; ik++)
    {
        ModuleBase::Sphbes::Spherical_Bessel(this->nr,
        ...
        ModuleBase::Integral::Simpson_Integral(this->nr,
                                               integrated_func,
...
#pragma omp parallel for schedule(static)
    ...
    for (int ik = 0; ik < nk; ik++)
    {
        ...
        psi_f_tmp *= pref;
#else
        const double psi_f_tmp = pref
                                 * BlasConnector::dot(this->nr,
                                                      ModuleBase::GlobalFunc::VECTOR_TO_PTR(r_tmp),
                                                      1,
                                                      ModuleBase::GlobalFunc::VECTOR_TO_PTR(jl[ik]),
                                                      1);
#endif
...
#else
        this->psi[ir] = pref
                        * BlasConnector::dot(this->nk,
                                             ModuleBase::GlobalFunc::VECTOR_TO_PTR(k_tmp),
                                             1,
                                             ModuleBase::GlobalFunc::VECTOR_TO_PTR(jl[ir]),
                                             1);
#endif
...
void Numerical_Orbital_Lm::plot(void) const
{
    ...
    if (GlobalV::MY_RANK == 0)
    {
        ...
        ssr << PARAM.globalv.global_out_dir << this->label << "/" << this->label << "-" << orbital_type << index_chi + 1
            << "-orbital-r.dat";

        ssk << PARAM.globalv.global_out_dir << this->label << "/" << this->label << "-" << orbital_type << index_chi + 1
            << "-orbital-k.dat";

        ssru << PARAM.globalv.global_out_dir << this->label << "/" << this->label << "-" << orbital_type
             << index_chi + 1 << "-orbital-ru.dat";
...
Numerical_Orbital_Lm Numerical_Orbital_Lm::operator*(double c) const
{
    Numerical_Orbital_Lm res = *this;
...
Numerical_Orbital_Lm operator*(double c, const Numerical_Orbital_Lm& orb)
{
    return orb * c;
}

Numerical_Orbital_Lm Numerical_Orbital_Lm::operator+(const Numerical_Orbital_Lm& rhs) const
{
    assert(psi.size() == rhs.psi.size());
    assert(psif.size() == rhs.psif.size());
    ...
    {
        res.psi[i] += rhs.psi[i];
        res.psir[i] += rhs.psir[i];
    }

    for (size_t i = 0; i < psif.size(); ++i)
    {
        res.psif[i] += rhs.psif[i];
        res.psik[i] += rhs.psik[i];
        res.psik2[i] += rhs.psik2[i];
    }
...
Numerical_Orbital_Lm& Numerical_Orbital_Lm::operator+=(const Numerical_Orbital_Lm& rhs)
{
    // 基本一致性检查
    assert(psi.size() == rhs.psi.size());
    assert(psif.size() == rhs.psif.size());
...
Numerical_Orbital_Lm& Numerical_Orbital_Lm::operator=(const Numerical_Orbital_Lm& rhs)
{
    ...
    label = rhs.label;
    index_atom_type = rhs.index_atom_type;
    angular_momentum_l = rhs.angular_momentum_l;
    index_chi = rhs.index_chi;
    ...
    r_radial = rhs.r_radial;
    k_radial = rhs.k_radial;
    rab = rhs.rab;
    ...
    psi_uniform = rhs.psi_uniform;
    dpsi_uniform = rhs.dpsi_uniform;
    ddpsi_uniform = rhs.ddpsi_uniform;
    nr_uniform = rhs.nr_uniform;
    dr_uniform = rhs.dr_uniform;
    zty = rhs.zty;

    return *this;
}

Numerical_Orbital_Lm& Numerical_Orbital_Lm::operator/=(double c)
{
    assert(c != 0.0);

...
void Sph_Bessel_Recursive::D2::cal_jlx_smallx( const int l_size, const size_t ix1_size, const size_t ix2_size )
{
	...
	for( int l=0; l!=l_size; ++l )
	{
		...
		for( size_t ix1=1; ix1<ix1_size; ++ix1 )
		{
			const size_t ix2_size_begin = std::max( static_cast<size_t>(1), jlx[l][ix1].size() );
			const size_t ix2_size_end = std::min( ix2_size, static_cast<size_t>(smallx_range/dx)/ix1 );
			if(jlx[l][ix1].size()<ix2_size_end)
				jlx[l][ix1].resize(ix2_size_end);
			for( size_t ix2=ix2_size_begin; ix2<ix2_size_end; ++ix2 )
			{
				const double x1 = ix1*ix2 * dx;
				jlx[l][ix1][ix2] = coeff*pow(x1,l);
			}
		}
	}	
}
	...
	for( int l=0; l!=l_size; ++l )
	{
		switch(l)
		{
			case 0:
				...
				{
					const size_t ix2_size_begin = std::max( static_cast<size_t>(1), jlx[l][ix1].size() );
					if(jlx[l][ix1].size()<ix2_size)
						jlx[l][ix1].resize(ix2_size);
					for( size_t ix2=ix2_size_begin; ix2<ix2_size; ++ix2 )
					{
						const double x1 = ix1*ix2 * dx;
						jlx[l][ix1][ix2] = sin(x1)/x1;
					}
				}
				break;
			case 1:
				...
				for( size_t ix1=1; ix1<ix1_size; ++ix1 )
				{
					...
					for( size_t ix2=ix2_size_begin; ix2<ix2_size; ++ix2 )
					{
						...
						jlx[l][ix1][ix2] = sin(x1)/x2-cos(x1)/x1;
					}
				}
				break;		
			default:
				...
				for( size_t ix1=1; ix1<ix1_size; ++ix1 )
				{
					...
					{
						const double x1 = ix1*ix2 * dx;
						jlx[l][ix1][ix2] = (2*l-1)/x1*jlx[l-1][ix1][ix2] - jlx[l-2][ix1][ix2];
					}
				}
				break;
		}

...
bool read_atom_species(std::ifstream& ifa, std::ofstream& ofs_running, UnitCell& ucell)
{
    ...
    if (ModuleBase::GlobalFunc::SCAN_LINE_BEGIN(ifa, "ATOMIC_SPECIES"))
    {
        ...
        for (int i = 0; i < ntype; i++)
        {
            ...
            if (!end && ss >> one_string && one_string[0] != '#')
            {
                ...
                {
                    GlobalV::ofs_warning << "unrecognized pseudopotential type: " << one_string
            ...
            ucell.atoms[i].flag_empty_element
                = (search(ucell.atom_label[i].begin(), ucell.atom_label[i].end(), bsse_label.begin(), bsse_label.end())
                   != ucell.atom_label[i].end())
                      ? true
                      : false;
        }
    }
    ...
    if (GlobalC::exx_info.info_global.cal_exx || PARAM.inp.rpa)
    {
        ...
        if (ModuleBase::GlobalFunc::SCAN_LINE_BEGIN(ifa, "Shrink_ABFS_ORBITAL"))
        {
            for (int i = 0; i < ntype; i++)
            {
                std::string ofile;
...
bool read_lattice_constant(std::ifstream& ifa, std::ofstream& ofs_running, Lattice& lat)
{
    ...
    if (latName == "none")
    {
        ...
        if (ModuleBase::GlobalFunc::SCAN_LINE_BEGIN(ifa, "LATTICE_PARAMETERS", true, false))
        {
        ...
        {
            ModuleBase::WARNING_QUIT("unitcell::read_lattice_constant", "Please set LATTICE_VECTORS in the STRU file");
        }
    ...
    {
        if (ModuleBase::GlobalFunc::SCAN_LINE_BEGIN(ifa, "LATTICE_VECTORS", true, false))
        {
            ModuleBase::WARNING_QUIT("unitcell::read_lattice_constant",

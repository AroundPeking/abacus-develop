#ifndef DIRECT_2D_COULOMB_EWALD_HPP
#define DIRECT_2D_COULOMB_EWALD_HPP
...
void Ewald_Vq<Tdata>::output_direct_2d_coulomb(const UnitCell& ucell,
                                               const double ecut_ry,
                                               const int kz_order,
                                               const int gamma_order)
{
    if (this->ewald_dimension != 2)
    {
        throw std::invalid_argument("Direct mixed-Fourier Coulomb requires Ewald dimension 2.");
    }
    if (!(ecut_ry > 0.0) || !std::isfinite(ecut_ry) || kz_order <= 0
        || gamma_order <= 0 || gamma_order % 2 != 0)
    {
        throw std::invalid_argument("Direct mixed-Fourier Coulomb received invalid quadrature controls.");
    }
    if (this->p_kv == nullptr
        || static_cast<int>(this->p_kv->kvec_c_full.size()) != this->nks0)
    {
        throw std::runtime_error("Direct G-sum Coulomb requires a complete full-q Cartesian mesh.");
    }
    ...
    {
        throw std::runtime_error("Direct G-sum Coulomb could not find the Ewald Fock parameters.");
    }
    const double chi = this->get_singular_chi(ucell, fock_params->second, 2.0);
    const double alpha = this->bare_multipole_scale;
    if (alpha <= 0.0)
    {
        throw std::runtime_error("Direct G-sum Coulomb requires a positive total Ewald Fock alpha.");
    }
    ...
    {
        throw std::runtime_error("Direct G-sum Coulomb auxiliary traversal size is inconsistent.");
    }
    ...
    if (!this->p_kv->kstars.empty())
    {
        ...
        for (std::size_t ibz = 0; ibz < this->p_kv->kstars.size(); ++ibz)
        {
            ...
            if (identity == this->p_kv->kstars[ibz].end())
            {
                throw std::runtime_error(
                    "Direct G-sum Ewald replacement could not find the identity member "
                    "of an IBZ q-star.");
            }
        ...
        if (mpi_rank == 0)
        {
            std::cout << "Direct mixed-Fourier Coulomb writes " << q_outputs.size()
    ...
    for (std::size_t requested = 0; requested < q_outputs.size(); ++requested)
    {
        ...
        const auto q_cart = q_output.q_cart;
        const auto q_direct = q_output.q_direct;
        const bool is_gamma = q_cart.norm2() <= 1.0e-20;
        Direct2dCoulomb::Result result;
        const auto lines = Direct2dCoulomb::enumerate_reciprocal_lines(
            ucell.G, q_direct, ucell.tpiba, ecut_ry);
        std::vector<Direct2dCoulomb::ReciprocalLine2D> regular_lines;
        regular_lines.reserve(lines.size());
        for (const auto& line: lines)
        {
            if (line.k_parallel_bohr > 1.0e-14)
            {
                regular_lines.push_back(line);
            }
        }
        auto points = Direct2dCoulomb::expand_mixed_fourier_points(
            regular_lines, ucell.tpiba, kz_order);
        if (is_gamma)
        {
            const auto gamma_plane = Direct2dCoulomb::gamma_plane_quadrature(
                ucell.G, ucell.tpiba, this->nmp, gamma_order, chi);
            auto gamma_points = Direct2dCoulomb::expand_gamma_zero_line(
                gamma_plane, ucell.tpiba, ecut_ry, kz_order);
            points.insert(points.end(), gamma_points.begin(), gamma_points.end());
        }
        const double lz_bohr = ucell.a3.norm() * ucell.lat0;
        result = Direct2dCoulomb::build_gram_matrix(
            basis, points, ucell.omega, lz_bohr, alpha);
        std::ostringstream stem;
        stem << "direct_2d_coulomb_iq" << iq;
        ...
        std::cout << "Direct G-sum Coulomb diagnostic: iq=" << iq
                  << " full_iq=" << q_output.full_iq
                  << " dimension=" << this->ewald_dimension
                  << " naux=" << result.matrix.dimension
                  << " regular_points=" << result.regular_g_count
                  << " ecut_ry=" << ecut_ry
                  << " kz_order=" << kz_order
                  << " gamma_plane_order=" << gamma_order
...
#endif
        const Direct2dCoulomb::MethodMetadata method_metadata{
            "direct_mixed_fourier",
            ecut_ry,
            kz_order,
            gamma_order,
            ...
            basis.size(),
            source_revision};
        const std::string filename = "librpa_2d_coulomb_method.dat";
        std::ofstream output(filename, std::ios::out | std::ios::trunc);
        if (!output.good())
        {
            throw std::runtime_error("Failed to open " + filename);
        }
        output << Direct2dCoulomb::format_method_metadata(method_metadata);
        if (!output.good())
        {

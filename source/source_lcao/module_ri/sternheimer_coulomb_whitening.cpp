...
std::vector<std::vector<double>> apply_sternheimer_channel_transform(
    ...
    const SternheimerCoulombWhitening& whitening)
{
    if (static_cast<int>(raw_channel_grid_values.size()) != whitening.raw_dimension
        || whitening.retained_rank <= 0)
    {
        throw std::invalid_argument(
            "Sternheimer channel-major transform dimension does not match its whitening metric.");
    }
    const std::size_t row_count = raw_channel_grid_values.front().size();
    for (const std::vector<double>& channel: raw_channel_grid_values)
    {
        if (channel.size() != row_count)
        {
            throw std::invalid_argument(
                "Sternheimer channel-major grid potentials have inconsistent sizes.");
        }
    }
    if (row_count > static_cast<std::size_t>(std::numeric_limits<int>::max()))
    {
        throw std::overflow_error("Sternheimer channel-major grid size exceeds the transform interface.");
    }

    std::vector<double> transformed_rows;
    {
        std::vector<double> raw_rows(row_count * raw_channel_grid_values.size(), 0.0);
#pragma omp parallel for schedule(static)
        for (std::size_t row = 0; row != row_count; ++row)
        {
            for (int raw = 0; raw != whitening.raw_dimension; ++raw)
            {
                raw_rows[row * raw_channel_grid_values.size() + static_cast<std::size_t>(raw)]
                    = raw_channel_grid_values[static_cast<std::size_t>(raw)][row];
            }
        }
        transformed_rows = apply_sternheimer_channel_transform(raw_rows,
    ...
    std::vector<std::vector<double>> transformed_channels(
        ...
        std::vector<double>(row_count, 0.0));
#pragma omp parallel for schedule(static)
    for (int retained = 0; retained != whitening.retained_rank; ++retained)
    {
        for (std::size_t row = 0; row != row_count; ++row)
        {
            transformed_channels[static_cast<std::size_t>(retained)][row]
                = transformed_rows[row * static_cast<std::size_t>(whitening.retained_rank)

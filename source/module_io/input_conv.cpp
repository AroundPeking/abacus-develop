...
//----------------------------------------------------------
    ...
    else if (dft_functional_lower == "cwp22")
    {
        ...
        GlobalC::exx_info.info_global.ccp_type
            = Conv_Coulomb_Pot_K::Ccp_Type::Erfc; // use the erfc(w|r-r'|), exx just has the short-range part
    }
    ...
    )
    {
        ...
        XC_Functional::set_hybrid_param(std::stod(PARAM.inp.exx_hybrid_alpha), std::stod(PARAM.inp.exx_hybrid_beta));
        ...
        GlobalC::exx_info.info_ri.exx_cs_inv_thr = PARAM.inp.exx_cs_inv_thr;

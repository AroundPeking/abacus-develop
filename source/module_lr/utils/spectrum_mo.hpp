                                                  ...
                                                  const std::vector<int> nvirt)
{
    ...
    psi::Psi<std::complex<double>> c_psi_ks(nks, pc.get_col_size(), pc.get_row_size(), kv.ngk, true);
    ...
    std::vector<ct::Tensor> vk(nks,
                               LR_Util::newTensor<std::complex<double>>({pmat.get_col_size(), pmat.get_row_size()}));
    for (int id = 0; id < 3; ++id)
    {
        ...
        for (int is = 0; is < nspin_tmp; ++is)
        {
            ...
            {
                hamilt::folding_HR(*vR.get_current_term_pointer(id),
                                   vk[ik].data<std::complex<double>>(),
                                   kv.kvec_d[ik],
                                   pmat.get_row_size(),
                                   1 /*column-major*/);
            }
        ...
        for (int is = 0; is < nspin_tmp; ++is)
        {
            ...
            {
                LR_Util::gather_2d_to_full(pmo,
                                           v_mo.data() + ik * pmo.get_local_size(),
                ...
                // LR_Util::print_value(velocity_mo.data()+(is * 3 * nk + id * nk + ik) * KS_num * KS_num, KS_num,
                // KS_num);
            }
        }
    } // id
                               ...
                               const K_Vectors& kv)
{
    ...
    for (int is = 0; is < nspin_tmp; ++is)
    {
        ...
        for (int ik = 0; ik < nk; ++ik)
        {
            ...
            ofs << std::setw(4) << "KS1" << std::setw(10) << "E1(eV)" << std::setw(6) << "KS2" << std::setw(10)
                << "E2(eV)" << std::setw(16) << "x" << std::setw(23) << "|x|^2" << std::setw(18) << "y" << std::setw(23)
                << "|y|^2" << std::setw(18) << "z" << std::setw(23) << "|z|^2" << std::setw(13) << "average"
                << std::endl;
            ...
            for (int i = 0; i < KS_num; ++i)
            {
                for (int j = i; j < KS_num; ++j)
                {
                    ...
                    double average = (std::norm(out_spectrum_mo[ipair]) + std::norm(out_spectrum_mo[ipair + step])
                                      + std::norm(out_spectrum_mo[ipair + 2 * step]))
                                     / 3.0;
                    ofs << std::setw(3) << i << std::setw(12) << std::setprecision(6)
                        << eig_ks[i + ik * KS_num] * ModuleBase::Ry_to_eV << std::setw(4) << j << std::setw(12)
                        << eig_ks[j + ik * KS_num] * ModuleBase::Ry_to_eV << std::setw(28) << out_spectrum_mo[ipair]
                        << std::setw(13) << std::norm(out_spectrum_mo[ipair]) << std::setw(28)
                        << out_spectrum_mo[ipair + step] << std::setw(13) << std::norm(out_spectrum_mo[ipair + step])
                        << std::setw(28) << out_spectrum_mo[ipair + 2 * step] << std::setw(13)
                        << std::norm(out_spectrum_mo[ipair + 2 * step]) << std::setw(13) << average << std::endl;
                }
            }
        }
    }
            ...
            for (int id = 0; id < 3; ++id)
            {
                ...
                for (int ib1 = 0; ib1 < KS_num; ++ib1)
                {
                    for (int ib2 = 0; ib2 < KS_num; ++ib2)
                    {
                        ...
                        ofs << std::setw(26) << std::fixed << std::setprecision(16)
                            << out_spectrum_mo[ipair].real() * HaBohrToEvAng << std::setw(26)
                            << out_spectrum_mo[ipair].imag() * HaBohrToEvAng << std::endl;
                    }
                }
            }
        }

...
在上面的单一指标方向中，这把与 \(D\) 相连的 AO 指标从 \(N_{\rm AO}\) 精确换成 \(N_{\rm occ}\)。不能把它笼统称为“第二个 AO 指标”。LibRI 的 `cal_Hs` 实际求和四个标签：`a0b0_a1b1`、`a0b0_a1b2`、`a0b0_a2b1` 和 `a0b0_a2b2`。若三中心张量写成 \(C_{a_0a_1a_2}\)，密度矩阵分别连接 \((a_1,b_1)\)、\((a_1,b_2)\)、\((a_2,b_1)\) 和 \((a_2,b_2)\)；因此两个 AO 腿都会在不同路径中成为收缩腿。

这意味着不能生成一份“固定投影第二腿”的标准 C-map，再交给未修改的 `cal_Hs`。在 `a1*` 路径中，这样做投影的是自由输出腿，因而可能保持 \(\mathrm{Tr}(DH)\) 却改变完整 \(H\)。正确的生产表示必须按每条标签只压缩当次与 \(D\) 相连的腿，或者直接使用周期 AO--occupied 乘积
...
并保留自由 AO 腿。该对象带电子动量 \(\mathbf k\) 和转移动量 \(\mathbf q\)，不能无损塞回只含一个原子对平移键的现有 C-map。对分数占据，根据密度矩阵本征值截断并单独报告占据截断误差。

对周期体系，占据分解必须在每个自旋和 \(\mathbf k\) 点的厄米非负 \(D^\sigma(\mathbf k)\) 上进行。不得对单个实空间 \(D(\mathbf R)\) 块独立做“占据分解”，因为该块未必是非负矩阵。只有带有完整标签和双动量语义的新收缩核可以把占据因子映射回 LibRI；逐块或单一 Fourier 指标的 C 投影只可作为局部代数诊断。
...
3. 先逐项验证 LibRI 四个 `a1/a2` 标签中哪条 AO 腿与 \(D\) 相连；禁止用一份固定腿投影的 C-map 代替这一步。
4. 在保留自由 AO 腿的标签感知或 \((\mathbf k,\mathbf q)\) AO--occupied 收缩中验证 \(D=OO^\dagger\)，并要求重建的 \(H_x,E_x\) 与原始 `CVCD` 在浮点精度内一致。
5. 检查 LibRI 现有收缩是否已经使用等价的占据低秩路径，防止重复计算收益。
6. 对 Coulomb 白化的 \(B_{Piv}\) 计算矩阵 SVD 上界、三阶 TT 的两组 Schmidt 谱和 discarded weight。
7. 扫描 \(10^{-8},10^{-7},10^{-6},10^{-5},10^{-4},10^{-3}\) 的白化残差或 discarded-weight 阈值。
...
- 分别覆盖四个 `a1b1/a1b2/a2b1/a2b2` 路径，检查每条路径只压缩与 \(D\) 相连的 AO 腿；
- 固定投影单个 C 张量腿的负面测试必须显示：能量相等不能替代完整 \(H_x\) 相等；
- \(C=0\)、\(D=0\)、秩一 \(C\)；

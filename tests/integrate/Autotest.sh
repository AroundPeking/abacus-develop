    ...
    if [ ! -d $dir ];then
        ...
        time {
            ...
            if [ $? -ne 0 ]; then
                ...
                # check the output
                if [ ! -d OUT.autotest ]; then
                    echo "No 'OUT.autotest' dir presented. Some errors may happened in ABACUS."
                    let fatal++
                    fatal_case_list+=$dir'\n'
                    fatal_detail_list+="$dir: OUT.autotest is missing\n"
                    case_status+=$dir' 0\n'
                elif test -z $g
                then
...
case_status=""
for dir in $testdir; do
    res="$result_dir/$dir.res"
    if [ ! -f "$res" ]; then
        echo -e "\e[0;31m[ERROR     ]\e[0m $dir did not produce a result summary.\n"
        let fatal++
        fatal_case_list+=$dir'\n'
        fatal_detail_list+="$dir: missing $res\n"
        continue
    fi
    while IFS= read -r line; do
        case "$line" in
            ...
            OK\ *) ok=$((ok + ${line#OK })) ;;
            FATAL\ *) fatal=$((fatal + ${line#FATAL })) ;;
            FAILED_LIST\ *) failed_case_list+="${line#FAILED_LIST }" ;;
            FATAL_LIST\ *) fatal_case_list+="${line#FATAL_LIST }" ;;
            FATAL_DETAIL\ *) fatal_detail_list+="${line#FATAL_DETAIL }" ;;
            STATUS\ *) case_status+="${line#STATUS }" ;;
        esac
    done < "$res"
done
